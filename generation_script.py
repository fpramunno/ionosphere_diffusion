...
ckpt = torch.load("/mnt/nas05/data01/francesco/progetto_simone/ionosphere/model_cond_generation/model_01236805.pth")
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model_ema.inner_model.load_state_dict(ckpt['model_ema'])
model_ema.to(device)

samples = generate_samples(model_ema, 10, device, cond_label=cond_label.unsqueeze(0).to(device), sampler="dpmpp_2m_sde")
...
np.save("/mnt/nas05/data01/francesco/progetto_simone/ionosphere/np_generated_data/sample.npy", samples.cpu().numpy())
# Save the condition
