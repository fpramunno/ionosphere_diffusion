...
#SBATCH -A sk035
#SBATCH --output=./logs/out/out_generate_2015_diffusion.log
#SBATCH --error=./logs/err/err_generate_2015_diffusion.log
...
# Fixed config
# =============================================================================
CSV_PATH="${IONO_HOME_ROOT:-/path/to/home_root}/data/ionosphere/l1_to_map_matched_2015_march_10_20_deduplicated.csv"
...
for PRED_START in "${PRED_STARTS[@]}"; do
    ...
    # CLASSIC
    OUTPUT_DIR="${IONO_DATA_ROOT:-/path/to/data_root}/results_2015_CLASSIC_AR50_pred${PRED_START_TAG}"
    mkdir -p $OUTPUT_DIR
    echo ">>> Running CLASSIC → $OUTPUT_DIR"
    python3 ${IONO_REPO:-/path/to/ionosphere_diffusion}/generate_data_v4.py \
        --model-type diffusion \
        --config ${IONO_REPO:-/path/to/ionosphere_diffusion}/configs/forecast_iono_15_big_cosine_solar_classic.json \
        --ckpt ${IONO_DATA_ROOT:-/path/to/data_root}/models_results/models_ViT_forecast_cond15_pred7_absolute_max_ddp_CLASSIC_v1_BS1/model_step_0200000.pth \
    ...
    # NOCOND
    OUTPUT_DIR="${IONO_DATA_ROOT:-/path/to/data_root}/results_2015_NOCOND_AR50_pred${PRED_START_TAG}"
    mkdir -p $OUTPUT_DIR
    echo ">>> Running NOCOND → $OUTPUT_DIR"
    python3 ${IONO_REPO:-/path/to/ionosphere_diffusion}/generate_data_v4.py \
        --model-type diffusion \
        --config ${IONO_REPO:-/path/to/ionosphere_diffusion}/configs/forecast_iono_15_big_cosine_solar_classic.json \
        --ckpt ${IONO_DATA_ROOT:-/path/to/data_root}/models_results/models_ViT_forecast_cond15_pred7_absolute_max_ddp_NOCOND_v1_BS1/model_step_0200000.pth \
        --output-dir $OUTPUT_DIR \
        --csv-path $CSV_PATH \
        --sequence-length $SEQUENCE_LENGTH \
        --normalization-type $NORMALIZATION_TYPE \
